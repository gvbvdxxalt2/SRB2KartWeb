...
function findWasmBinary() {
  return locateFile('srb2kart.wasm');
}
  ...
  init() {
        ...
        _glGetBooleanv = _emscripten_glGetBooleanv = (pname, p) => {
          ...
          if (attrib !== null) {
            ...
            HEAP8[p] = result === true ? 1 : 0;checkInt8(result === true ? 1 : 0);
            return;
          }
        ...
        _glGetIntegerv = _emscripten_glGetIntegerv = (pname, params) => {
          ...
          switch (pname) {
            ...
            case 0x8B4A: { // GL_MAX_VERTEX_UNIFORM_COMPONENTS_ARB
              ...
              HEAP32[((params)>>2)] = result*4;checkInt32(result*4); // GLES gives num of 4-element vectors, GL wants individual components, so multiply
              return;
            }
            ...
            case 0x807A: { // GL_VERTEX_ARRAY_SIZE
              ...
              HEAP32[((params)>>2)] = attribute ? attribute.size : 0;checkInt32(attribute ? attribute.size : 0);
              return;
            }
            case 0x807B: { // GL_VERTEX_ARRAY_TYPE
              var attribute = GLImmediate.clientAttributes[GLImmediate.VERTEX];
              HEAP32[((params)>>2)] = attribute ? attribute.type : 0;checkInt32(attribute ? attribute.type : 0);
              return;
            }
            case 0x807C: { // GL_VERTEX_ARRAY_STRIDE
              var attribute = GLImmediate.clientAttributes[GLImmediate.VERTEX];
              HEAP32[((params)>>2)] = attribute ? attribute.stride : 0;checkInt32(attribute ? attribute.stride : 0);
              return;
            }
            ...
            case 0x0D32: { // GL_MAX_CLIP_PLANES
              HEAP32[((params)>>2)] = GLEmulation.MAX_CLIP_PLANES;checkInt32(GLEmulation.MAX_CLIP_PLANES); // all implementations need to support at least 6
              return;
            }
            case 0x0BA0: { // GL_MATRIX_MODE
              HEAP32[((params)>>2)] = GLImmediate.currentMatrix + 0x1700;checkInt32(GLImmediate.currentMatrix + 0x1700);
              return;
            }
            case 0x0BC1: { // GL_ALPHA_TEST_FUNC
              HEAP32[((params)>>2)] = GLEmulation.alphaTestFunc;checkInt32(GLEmulation.alphaTestFunc);
              return;
            }
          }
  ...
  spawnTexEnvJIT:() => {
        ...
        return {
          ...
          hook_getTexEnviv(target, pname, param) {
            ...
            switch (pname) {
              case GL_TEXTURE_ENV_MODE:
                HEAP32[((param)>>2)] = env.mode;checkInt32(env.mode);
                return;
  
              case GL_TEXTURE_ENV_COLOR:
                HEAP32[((param)>>2)] = Math.max(Math.min(env.envColor[0]*255, 255, -255));checkInt32(Math.max(Math.min(env.envColor[0]*255, 255, -255)));
                HEAP32[(((param)+(1))>>2)] = Math.max(Math.min(env.envColor[1]*255, 255, -255));checkInt32(Math.max(Math.min(env.envColor[1]*255, 255, -255)));
                HEAP32[(((param)+(2))>>2)] = Math.max(Math.min(env.envColor[2]*255, 255, -255));checkInt32(Math.max(Math.min(env.envColor[2]*255, 255, -255)));
                HEAP32[(((param)+(3))>>2)] = Math.max(Math.min(env.envColor[3]*255, 255, -255));checkInt32(Math.max(Math.min(env.envColor[3]*255, 255, -255)));
                return;
  
              case GL_COMBINE_RGB:
                HEAP32[((param)>>2)] = env.colorCombiner;checkInt32(env.colorCombiner);
                return;
  
              case GL_COMBINE_ALPHA:
                HEAP32[((param)>>2)] = env.alphaCombiner;checkInt32(env.alphaCombiner);
                return;
  
              case GL_SRC0_RGB:
                HEAP32[((param)>>2)] = env.colorSrc[0];checkInt32(env.colorSrc[0]);
                return;
  
              case GL_SRC1_RGB:
                HEAP32[((param)>>2)] = env.colorSrc[1];checkInt32(env.colorSrc[1]);
                return;
  
              case GL_SRC2_RGB:
                HEAP32[((param)>>2)] = env.colorSrc[2];checkInt32(env.colorSrc[2]);
                return;
  
              case GL_SRC0_ALPHA:
                HEAP32[((param)>>2)] = env.alphaSrc[0];checkInt32(env.alphaSrc[0]);
                return;
  
              case GL_SRC1_ALPHA:
                HEAP32[((param)>>2)] = env.alphaSrc[1];checkInt32(env.alphaSrc[1]);
                return;
  
              case GL_SRC2_ALPHA:
                HEAP32[((param)>>2)] = env.alphaSrc[2];checkInt32(env.alphaSrc[2]);
                return;
  
              case GL_OPERAND0_RGB:
                HEAP32[((param)>>2)] = env.colorOp[0];checkInt32(env.colorOp[0]);
                return;
  
              case GL_OPERAND1_RGB:
                HEAP32[((param)>>2)] = env.colorOp[1];checkInt32(env.colorOp[1]);
                return;
  
              case GL_OPERAND2_RGB:
                HEAP32[((param)>>2)] = env.colorOp[2];checkInt32(env.colorOp[2]);
                return;
  
              case GL_OPERAND0_ALPHA:
                HEAP32[((param)>>2)] = env.alphaOp[0];checkInt32(env.alphaOp[0]);
                return;
  
              case GL_OPERAND1_ALPHA:
                HEAP32[((param)>>2)] = env.alphaOp[1];checkInt32(env.alphaOp[1]);
                return;
  
              case GL_OPERAND2_ALPHA:
                HEAP32[((param)>>2)] = env.alphaOp[2];checkInt32(env.alphaOp[2]);
                return;
  
              case GL_RGB_SCALE:
                HEAP32[((param)>>2)] = env.colorScale;checkInt32(env.colorScale);
                return;
  
              case GL_ALPHA_SCALE:
                HEAP32[((param)>>2)] = env.alphaScale;checkInt32(env.alphaScale);
                return;
          ...
          switch (pname) {
            case 0x8B8D: { // GL_CURRENT_PROGRAM
              ...
              if (cur == GLImmediate.fixedFunctionProgram) {
                ...
                HEAP32[((params)>>2)] = 0;checkInt32(0);
                return;
              }
              break;
            }
            ...
            if ((srcStride & 3) == 0 && (attr.sizeBytes & 3) == 0) {
              for (var j = 0; j < count; j++) {
                for (var k = 0; k < attr.sizeBytes; k+=4) { // copy in chunks of 4 bytes, our alignment makes this possible
                  ...
                  HEAP32[(((start + attr.offset)+(bytes*j + k))>>2)] = val;checkInt32(val);
                }
              }
            } else {
              for (var j = 0; j < count; j++) {
 ...
 */
  ...
  var emulGlGenVertexArrays = (n, vaos) => {
      for (var i = 0; i < n; i++) {
        ...
        HEAP32[(((vaos)+(i*4))>>2)] = id;checkInt32(id);
      }
    };
  ...
  var _emscripten_glGetObjectParameteriv = (id, type, result) => {
      if (GL.programs[id]) {
        if (type == 0x8B84) { // GL_OBJECT_INFO_LOG_LENGTH_ARB
          ...
          if (log === null) log = '(unknown error)';
          HEAP32[((result)>>2)] = log.length;checkInt32(log.length);
          return;
        }
        ...
        if (type == 0x8B84) { // GL_OBJECT_INFO_LOG_LENGTH_ARB
          ...
          HEAP32[((result)>>2)] = source.length;checkInt32(source.length);
          return;
        }
  ...
  var _emscripten_glGetPointerv = (name, p) => {
      ...
      HEAP32[((p)>>2)] = attribute ? attribute.pointer : 0;checkInt32(attribute ? attribute.pointer : 0);
    };
...
var dynCall_viiffiifii = makeInvalidEarlyAccess('dynCall_viiffiifii');
...
var dynCall_vddd = makeInvalidEarlyAccess('dynCall_vddd');
var dynCall_vdddd = makeInvalidEarlyAccess('dynCall_vdddd');
var dynCall_vdddddd = makeInvalidEarlyAccess('dynCall_vdddddd');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['dynCall_viiffiifii'] != 'undefined', 'missing Wasm export: dynCall_viiffiifii');
  ...
  assert(typeof wasmExports['dynCall_vddd'] != 'undefined', 'missing Wasm export: dynCall_vddd');
  assert(typeof wasmExports['dynCall_vdddd'] != 'undefined', 'missing Wasm export: dynCall_vdddd');
  assert(typeof wasmExports['dynCall_vdddddd'] != 'undefined', 'missing Wasm export: dynCall_vdddddd');
  ...
  dynCall_viiffiifii = dynCalls['viiffiifii'] = createExportWrapper('dynCall_viiffiifii', wasmExports['dynCall_viiffiifii'], 10);
  ...
  dynCall_vddd = dynCalls['vddd'] = createExportWrapper('dynCall_vddd', wasmExports['dynCall_vddd'], 4);
  dynCall_vdddd = dynCalls['vdddd'] = createExportWrapper('dynCall_vdddd', wasmExports['dynCall_vdddd'], 5);
  dynCall_vdddddd = dynCalls['vdddddd'] = createExportWrapper('dynCall_vdddddd', wasmExports['dynCall_vdddddd'], 7);
