...
emcmake cmake .. \
    ...
    "-DCMAKE_C_FLAGS=-DEMSCRIPTEN -mbulk-memory -s USE_ZLIB=1 -s USE_LIBPNG=1 -flto -msimd128 -DMASTERSERVER -DHAVE_SDLNET -std=gnu17 -s USE_SDL=2 -s SDL2_MIXER_FORMATS=['ogg','wav'] -s USE_SDL_MIXER=2 -s USE_SDL_IMAGE=2 -s USE_SDL_TTF=2 -s USE_SDL_NET=2 -g2" \
    "-DCMAKE_CXX_FLAGS=-DEMSCRIPTEN -mbulk-memory -s USE_ZLIB=1 -s USE_LIBPNG=1 -O3 -flto -msimd128 -s USE_SDL=2 -s SDL2_MIXER_FORMATS=['ogg','wav'] -s USE_SDL_MIXER=2 -s USE_SDL_IMAGE=2 -s USE_SDL_TTF=2 -s USE_SDL_NET=2" \
    "-DCMAKE_EXE_LINKER_FLAGS=-mbulk-memory -s USE_ZLIB=1 -s USE_LIBPNG=1 -O3 -msimd128 -flto -sINITIAL_MEMORY=134217728 -sALLOW_MEMORY_GROWTH=1 -sMAXIMUM_MEMORY=2147483648 -sWASM_BIGINT --trace -s EMBIND_STD_STRING_IS_UTF8=1 -s SDL2_MIXER_FORMATS=['ogg','wav'] -DEMSCRIPTEN -DHAVE_SDLNET -s ASSERTIONS=2 -gsource-map -g2 -s STACK_SIZE=5242880 -s USE_SDL=2 -s USE_SDL_IMAGE=2 -s USE_SDL_MIXER=2 -s USE_SDL_TTF=2 -s USE_ZLIB=1 -s USE_SDL_NET=1 -s USE_LIBPNG=1 -s ALLOW_MEMORY_GROWTH=1 -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap','UTF8ToString','stringToUTF8','lengthBytesUTF8','intArrayFromString','FS','stackAlloc','callMain','wasmMemory'] -gsource-map -g -s ENVIRONMENT=web,node -lidbfs.js" \
    -DZLIB_FOUND=TRUE \
    -DZLIB_LIBRARY=z \
    -DPNG_FOUND=TRUE \
    -DPNG_LIBRARY=png \
...
cd bin
if [ -f "srb2kart" ] && [ ! -f "srb2kart.js" ]; then
    mv srb2kart srb2kart.js
fi