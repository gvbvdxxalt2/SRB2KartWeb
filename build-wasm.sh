...
emcmake cmake .. -DCMAKE_BUILD_TYPE=Release \
    ...
    "-DCMAKE_C_FLAGS=-DEMSCRIPTEN -DMASTERSERVER -s USE_SDL=2 -s USE_SDL_IMAGE=2 -s USE_SDL_MIXER=2 -s USE_SDL_TTF=2 -s USE_ZLIB=1 -s USE_LIBPNG=1 -s USE_SDL_NET=1 -std=gnu17 -I$VCPKG_INC" \
    "-DCMAKE_CXX_FLAGS=-DEMSCRIPTEN -s USE_SDL=2 -I$VCPKG_INC" \
    "-DCMAKE_EXE_LINKER_FLAGS=-DEMSCRIPTEN -s ASSERTIONS=2 -s STACK_OVERFLOW_CHECK=2 -gsource-map -g2 -s USE_SDL=2 -s USE_SDL_IMAGE=2 -s USE_SDL_MIXER=2 -s USE_SDL_TTF=2 -s USE_ZLIB=1 -s USE_SDL_NET=1 -s USE_LIBPNG=1 -s ALLOW_MEMORY_GROWTH=1 -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap','UTF8ToString','stringToUTF8','lengthBytesUTF8','intArrayFromString','FS','stackAlloc','callMain','wasmMemory'] -s ENVIRONMENT=web,node -lidbfs.js -L$VCPKG_LIB" \
        "-DCMAKE_EXE_LINKER_FLAGS=-DEMSCRIPTEN -s ASSERTIONS=2 -s STACK_OVERFLOW_CHECK=2 -gsource-map -g2 -s STACK_SIZE=5242880 -s USE_SDL=2 -s USE_SDL_IMAGE=2 -s USE_SDL_MIXER=2 -s USE_SDL_TTF=2 -s USE_SDL_NET=1 -s USE_ZLIB=1 -s USE_LIBPNG=1 -s ALLOW_MEMORY_GROWTH=1 -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap','UTF8ToString','stringToUTF8','lengthBytesUTF8','intArrayFromString','FS','stackAlloc','callMain','wasmMemory'] -s ENVIRONMENT=web,node -lidbfs.js -L$VCPKG_LIB" \
